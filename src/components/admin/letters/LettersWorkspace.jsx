...
import DeveloperBroadcastRecovery from './DeveloperBroadcastRecovery.jsx'
...
export function LetterEditor({ working, readOnly, saveState, undoCount, redoCount, previewMode, setPreviewMode, renderedPreview, previewLoading, previewError, flowStep, selectedBlockId, selectedBlock, versions, busy, templateName, setTemplateName, palette, onBack, onUpdateLetter, onUpdateDesign, onUndo, onRedo, onSave, onReloadConflict, onChooseRecipients, onAddBlock, onInsertBlock, onSelectBlock, onMoveBlock, onChangeBlock, onDuplicateBlock, onDeleteBlock, renderFlowPanel, onRestoreVersion, onSaveTemplate, formatStatus, formatDate }) {
  return (
    <section className="pwc-letters28-builder">
      <header className="pwc-letters28-topbar"><button type="button" className="is-back" onClick={onBack}>← Letters</button><div className="pwc-letters28-title-fields"><input aria-label="Internal letter title" value={working.title} onChange={(event) => onUpdateLetter({ title: event.target.value })} /><input aria-label="Email subject" value={working.subject || ''} onChange={(event) => onUpdateLetter({ subject: event.target.value })} placeholder="Email subject" /></div><div className="pwc-letters28-save-state" role="status" aria-live="polite"><span className={`is-${saveState}`}>{saveState === 'saved' ? 'Saved' : saveState === 'saving' ? 'Saving…' : saveState === 'conflict' ? 'Newer version found' : saveState === 'error' ? 'Save error' : 'Unsaved'}</span><small>Revision {working.autosave_revision}</small>{saveState === 'conflict' && <button type="button" onClick={onReloadConflict}>Reload saved version</button>}</div><div className="pwc-letters28-top-actions"><button type="button" onClick={onUndo} disabled={!undoCount || readOnly}>Undo</button><button type="button" onClick={onRedo} disabled={!redoCount || readOnly}>Redo</button><div role="group" aria-label="Preview mode"><button type="button" aria-pressed={previewMode === 'edit'} className={previewMode === 'edit' ? 'is-active' : ''} onClick={() => setPreviewMode('edit')}>Edit</button><button type="button" aria-pressed={previewMode === 'desktop'} className={previewMode === 'desktop' ? 'is-active' : ''} onClick={() => setPreviewMode('desktop')}>Desktop</button><button type="button" aria-pressed={previewMode === 'mobile'} className={previewMode === 'mobile' ? 'is-active' : ''} onClick={() => setPreviewMode('mobile')}>Mobile</button><button type="button" aria-pressed={previewMode === 'plain'} className={previewMode === 'plain' ? 'is-active' : ''} onClick={() => setPreviewMode('plain')}>Text</button></div><button type="button" onClick={onSave} disabled={readOnly || saveState === 'saving' || saveState === 'conflict'}>Save now</button><button type="button" className="is-primary" onClick={onChooseRecipients} disabled={readOnly || !working.subject || saveState === 'conflict'}>Choose recipients →</button></div></header>
