  ...
  return (
    <div className="pwc-letters28-settings">
      ...
      <div className="pwc-letters28-settings-fields">
        ...
        {block.type === 'image' && <><AssetVaultPicker value={content.assetId || ''} onChange={(assetId) => changeContent('assetId', assetId)} label="Choose newsletter image" /><Field label="Alternative text"><input value={content.alt || ''} onChange={(event) => changeContent('alt', event.target.value)} /></Field><Field label="Caption"><input value={content.caption || ''} onChange={(event) => changeContent('caption', event.target.value)} /></Field><Field label="Image width"><input type="range" min="20" max="100" value={settings.width || 100} onChange={(event) => changeSetting('width', Number(event.target.value))} /><small>{settings.width || 100}%</small></Field></>}
        ...
        {block.type === 'video_preview' && <><AssetVaultPicker value={content.assetId || ''} onChange={(assetId) => changeContent('assetId', assetId)} label="Choose optional video preview image" /><Field label="Video title"><input value={content.title || ''} onChange={(event) => changeContent('title', event.target.value)} /></Field><Field label="Video URL" wide><input type="url" value={content.url || ''} onChange={(event) => changeContent('url', event.target.value)} /></Field></>}
        {block.type === 'resource' && <><AssetVaultPicker value={content.assetId || ''} onChange={(assetId) => changeContent('assetId', assetId)} label="Choose downloadable resource" /><Field label="Resource title"><input value={content.title || ''} onChange={(event) => changeContent('title', event.target.value)} /></Field><Field label="Description" wide><textarea rows="3" value={content.description || ''} onChange={(event) => changeContent('description', event.target.value)} /></Field></>}
